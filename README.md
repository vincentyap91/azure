...
## Local preview (for reviewers)

- **Dev:** `npm install` then `npm run dev`
- **Production build:** `npm run build`, then `npm run preview` (or serve the whole `dist` folder with any static server). Sending only `dist/index.html` without `dist/assets` will not work for the default build.
